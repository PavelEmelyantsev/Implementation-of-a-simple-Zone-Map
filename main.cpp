...
//    	printf("%d %s\n", queries[i], (zones.query(queries[i]) == true ? "found" : "not found" ));
	}
	
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    unsigned long long point_query_time = duration.count();
    std::cout << "Time taken to perform point queries from zonemap = " << point_query_time << " microseconds for " << queries.size() << " queries" << endl;
  }
  else if (test_case == "test_rq_1")
  {
    ...
    int low = zones.get_element(n / 10);
    int high = zones.get_element(n * 2 / 10);
...
//	}
  ...
  else if (test_case == "test_rq_2")
  {
    ...
    int low = zones.get_element(n * 3 / 10);
    int high = zones.get_element(n * 4 / 10);
...
//	}
  ...
  else if (test_case == "test_rq_3")
  {
    ...
    int low = zones.get_element(n * 6 / 10);
    int high = zones.get_element(n * 7 / 10);
...
//	}
  ...
  else if (test_case == "test_rq_4")
  {
    ...
    int low = zones.get_element(n * 8 / 10);
    int high = zones.get_element(n * 9 / 10);
